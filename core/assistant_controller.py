...
class AssistantController:
    ...
    def __init__(self):
        ...
        # UI hook for wakeword (OrbitOS sets this)
        ...
        self.wake_engine = WakeWordEngine(self.on_wake_word)
    ...
    def on_wake_word(self):
        """
        Called by WakeWordEngine when the wake word is detected.
        If UI registered on_wake, let it decide (assistant vs AI).
        Otherwise, use the original assistant-only behavior.
        """
        if self._busy:
            return
        ...
        # Fallback: original assistant flow
        self._busy = True
    ...
    def listen_voice_only_async(self, callback: Callable[[Optional[str]], None]) -> None:
        """
        Just transcribe one utterance (listen_once) and return the text,
        without running the command engine or speaking anything.
        Callback is always invoked on the Qt main thread.
        Used by AI mode wakeword and AI voice button.
        """
        if self._busy:
            return
        ...
        def worker():
            try:
                self._busy = True
                QTimer.singleShot(0, lambda: self._set_state("listening"))
                text = listen_once()

                def fire_cb():
                    callback(text if text else None)

                QTimer.singleShot(0, fire_cb)
            finally:
                self._busy = False
    ...
    def _handle_command(self, text: str):
        ...
        if rtype == "append_note":
            base_dir = str(self._working_dir)
            note_text = result.get("note_text", text)
            reply = notes_skill.append_note(note_text, base_dir=base_dir)
            self._speak(reply)
            return

        if rtype == "read_notes":
            base_dir = str(self._working_dir)
            notes = notes_skill.read_notes(base_dir=base_dir)
            self._speak(notes)
            if callable(self.on_message) and notes:
                self.on_message(notes)
            return
