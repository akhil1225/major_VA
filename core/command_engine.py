...
def extract_note_text(text: str) -> str:
    """
    Extract the note content from the user's sentence.
    The model only predicts APPEND_NOTE; we pull the actual note
    text rule-based from the raw utterance.
    """
    original = text.strip()
    ...
    for trig in triggers:
        if lower.startswith(trig):
            # slice original so we keep original casing/words
            return original[len(trig):].strip()

    # fallback: if we didn't match any trigger, just return the whole thing
    return original
...
def process_command(text: str) -> Dict:
    ...
    if intent == "APPEND_NOTE":
        # you can pass full text; extraction is optional
        return {"type": "append_note", "text": text}
