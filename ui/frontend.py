    ...
    def _build_animations(self):
        ...
        for i, bar in enumerate(self.bars):
            ...
            for anim in (min_anim, max_anim):
                ...
                anim.setEasingCurve(QEasingCurve.InOutSine)  # type: ignore[attr-defined]
                anim.setLoopCount(-1)
...
class ChatBubble(QFrame):
    def __init__(self, text: str, is_user: bool):
        ...
        label.setTextInteractionFlags(Qt.TextSelectableByMouse)  # type: ignore[attr-defined]

        label.setSizePolicy(
            QSizePolicy.Expanding,  # type: ignore[attr-defined]
            QSizePolicy.Preferred,  # type: ignore[attr-defined]
        )
        max_width = int(self.parentWidget().width() * 0.7) if self.parentWidget() else 480
...
class OrbitOS(QWidget):
    ...
    def __init__(self):
        ...
        # Route wakeword to assistant or AI depending on current_mode
        self.controller.on_wake = self._on_wake_from_engine

        # connect assistant voice pipeline → UI (assistant mode)
    ...
    def _build_ui(self):
        ...
        sidebar_layout.addWidget(self.toggle_sidebar_btn, alignment=Qt.AlignLeft)  # type: ignore[attr-defined]
        ...
        self.title_label.setFont(QFont("Segoe UI", 22, QFont.Bold))  # type: ignore[attr-defined]
        ...
        assistant_layout.addWidget(self.visualizer, 0, Qt.AlignCenter)  # type: ignore[attr-defined]

        self.assistant_scroll = QScrollArea()
        self.assistant_scroll.setWidgetResizable(True)
        self.assistant_scroll.setStyleSheet("border: none;")
        ...
        self.assistant_scroll.setWidget(self.chat_host)
        assistant_layout.addWidget(self.assistant_scroll, 1)
        ...
        ai_title.setFont(QFont("Segoe UI", 16, QFont.Bold))  # type: ignore[attr-defined]

        ai_subtitle = QLabel(
            "Ask anything, keep multiple chats, rename and manage them."
        )
        ...
        self.ai_sidebar.setContextMenuPolicy(Qt.CustomContextMenu)  # type: ignore[attr-defined]
        ...
        self.ai_voice_btn = QPushButton("🎤 Voice")
        self.ai_voice_btn.setFixedHeight(32)
        self.ai_voice_btn.setFixedWidth(90)
        self.ai_voice_btn.clicked.connect(self._run_ai_voice)
        ...
        ai_input_row.addWidget(self.ai_voice_btn)
        ...
        self.sidebar_anim.setEasingCurve(QEasingCurve.InOutCubic)  # type: ignore[attr-defined]
    ...
    def scroll_to_bottom(self):
        ...
        bar = self.assistant_scroll.verticalScrollBar()
        bar.setValue(bar.maximum())

    # ASSISTANT MODE INPUT
    ...
    # ASSISTANT VOICE (wakeword + button)
    ...
    # AI VOICE (button; logs into AI chat)

    def _run_ai_voice(self):
        """
        Voice input dedicated to AI mode (button).
        Uses pure STT, logs request in current chat, then calls LLM.
        """
        self.set_mode("ai")
        self.controller.pause_wake_word()
        self._pulse_visualizer()

        def after_stt(text: str | None):
            if text:
                if not self.current_ai_chat_id:
                    self._create_new_ai_chat()
                assert self.current_ai_chat_id is not None
                ...
                self._add_ai_message(f"[Voice Button] {text}", is_user=True)
                ai_reply = self.controller.run_ai_chat(chat_id, text, model=None)
                if ai_reply:
                    self._add_ai_message(ai_reply, is_user=False)

            QTimer.singleShot(0, self.controller.resume_wake_word)

        self.controller.listen_voice_only_async(after_stt)

    # WAKEWORD ROUTING

    def _on_wake_from_engine(self):
        """
        Called when the global wake word fires.
        Route based on current_mode.
        """
        if self.current_mode == "assistant":
            # Original assistant behavior: "Yes?" + command engine
            if self.controller._busy:
                return
            self.controller._busy = True
            self.controller._set_state("wake")
            self.controller._speak("Yes?")
            self.controller._listen_async()
        else:
            # AI mode: treat wake as an AI prompt
            self._run_ai_wake_flow()

    def _run_ai_wake_flow(self):
        """
        Wake word in AI mode:
        - Listen once (STT only)
        - Log in current AI chat
        - Send to LLM
        """
        self.set_mode("ai")
        self._pulse_visualizer()
        ...
        def after_stt(text: str | None):
            ...
            self._add_ai_message(f"[AI Wakeword] {text}", is_user=True)
            reply = self.controller.run_ai_chat(chat_id, text, model=None)
            if reply:
                self._add_ai_message(reply, is_user=False)
    ...
    def _load_ai_sidebar(self):
        ...
        for chat in self.controller.ai_store.chats:
            ...
            item.setData(Qt.UserRole, chat.id)  # type: ignore[attr-defined]
    ...
    def _on_ai_chat_selected(self, item: QListWidgetItem):
        chat_id = item.data(Qt.UserRole)  # type: ignore[attr-defined]
...
def launch_ui():
    print("Launching OrbitOS UI...")
    app = QApplication(sys.argv)
    win = OrbitOS()
    print("OrbitOS window created")
    win.show()
    sys.exit(app.exec())
